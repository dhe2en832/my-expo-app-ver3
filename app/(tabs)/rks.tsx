...
export default function RKSPage() {
  ...
  const [modalType, setModalType] = useState<"unscheduled" | "baru" | null>(null);
  ...
  const [selectedRksForMemo, setSelectedRksForMemo] = useState<RKS | null>(null);
  ...
  const tabToRange: Record<number, RangeKey> = { 0: "today", 1: "week", 2: "month" };
  ...
  const toggleFab = () => {
    ...
    if (!fabOpen) {
      ...
      Animated.spring(fabAnim, { toValue: 0, useNativeDriver: true, friction: 5 }).start();
    }
    setFabOpen(!fabOpen);
  };
  ...
  const filterByRange = useCallback((list: RKS[]) => {
    const now = new Date();
    if (range === "today") {
  ...
  const handleCheckOut = async (rks: RKS) => {
    try {
      if (!rks.checkIn) {
        Alert.alert("Belum Check In", "Anda harus melakukan Check In terlebih dahulu.");
        return;
      }
      setCheckingInId(rks.id);
      let { status } = await Location.requestForegroundPermissionsAsync();
      ...
      if (updateResp?.success && updateResp.rks) {
        setRksList((prev) => prev.map((x) => (x.id === rks.id ? updateResp.rks : x)));
      } else {
        setRksList((prev) =>
          prev.map((x) =>
            x.id === rks.id
              ? { ...x, checkOut: updateData.checkOut, duration: durationMin, status: "completed" } as RKS
              : x
          )
        );
      }
      const photoForCheckout = (rks.checkIn && (rks.checkIn.photo as any)) || "[no-photo]";
      ...
      Alert.alert("Check Out berhasil", `Durasi kunjungan: ${durationMin} menit`);
    } catch (err) {
  ...
  const proceedWithUnscheduledVisit = async (lat: number, lon: number, acc: number) => {
    ...
    try {
      ...
      if (res.success && res.rks) {
        ...
        const errorMessage = (res as any).error || "Gagal membuat kunjungan tambahan.";
  ...
  const handleOpenBesiKompetitor = (rks: { id: string; customerName: string }) => {
    router.push({
  ...
  const renderItem = ({ item }: { item: RKS }) => {
    ...
    return (
      ...
      //     <Text style={{ color: "#888", fontSize: 12, marginTop: 2 }}>No. Cust: {item.customerId}</Text> */}
      //       <View>
      //         <Text style={{ color: "#888", fontSize: 12 }}>No. Customer: {item.customerId}</Text>
      //         <Text style={styles.title}>{item.customerName}</Text>
      //       </View>
      //     <Text style={{ color: "#666" }}>{item.scheduledTime || ""}</Text>
      //   </View>
      //   <Text style={{ color: "#666", marginTop: 6 }}>{item.customerAddress}</Text>
      //   <View style={{ flexDirection: "row", marginTop: 12, gap: 8, flexWrap: 'wrap' }}>
      //     {canCheckIn && (
      //       <TouchableOpacity
      //         style={[
      //           styles.primaryButton,
      //           (!isToday || checkingInId !== null || (isAnyCheckedIn && !canCheckOut)) && { opacity: 0.5 }
      //         ]}
      //         onPress={() => handleCheckIn(item)}
      //         disabled={!isToday || checkingInId !== null || (isAnyCheckedIn && !canCheckOut)}
      //       >
      //         <LinearGradient
      ...
      //               <Text style={[styles.primaryButtonText, { marginLeft: 8 }]}>Sedang proses...</Text>
      //             </View>
      //           ) : (
      ...
      //           <Text style={[styles.primaryButtonText, { marginLeft: 4 }]}>Check Out</Text>
      //         </TouchableOpacity>
      //         <TouchableOpacity
      ...
      //           <Text style={[styles.primaryButtonText, { marginLeft: 4 }]}>SO</Text>
      //         </TouchableOpacity>
      //         <TouchableOpacity
      ...
      //           <Text style={[styles.primaryButtonText, { marginLeft: 4 }]}>Catatan</Text>
      //         </TouchableOpacity>
      //         <TouchableOpacity
      ...
      //           <Text style={[styles.primaryButtonText, { marginLeft: 4 }]}>Besi</Text>
      //         </TouchableOpacity>
      //         <TouchableOpacity
      //           style={[styles.primaryButton, { backgroundColor: '#009688' }]}
      //           onPress={() => handleTagihan(item.customerId, item.customerName)}
      //         >
      //           <MaterialIcons name="receipt" size={20} color="#fff" />
      //           <Text style={[styles.primaryButtonText, { marginLeft: 4 }]}>Tagihan</Text>
      //         </TouchableOpacity>
      //       </>
      //     )}
  ...
  const SceneList = () => {
    ...
    if (loading) {
      return <View style={{ padding: 20 }}><ActivityIndicator size="large" color="#667eea" /></View>;
    }
    if (filtered.length === 0) {
      return <View style={{ padding: 20 }}><Text style={{ color: "#999" }}>Tidak ada RKS pada rentang ini.</Text></View>;
    }
    return <FlatList data={filtered} keyExtractor={(i) => i.id} renderItem={renderItem} contentContainerStyle={{ padding: 12 }} />;
  };
  ...
  return (
    <View style={styles.container}>
      ...
      >
        <Animated.View style={[styles.fabOption, unscheduledStyle]}>
          <Animated.View style={{ opacity: tooltipAnim, marginBottom: 4, right: 60 }}>
            <View style={styles.tooltip}><Text style={styles.tooltipText}>Kunjungan Tambahan</Text></View>
          </Animated.View>
          <TouchableOpacity
            style={styles.fabOptionButton}
            onPress={() => { setModalType("unscheduled"); toggleFab(); }}
        ...
        <Animated.View style={[styles.fabOption, newCustomerStyle]}>
          ...
          <TouchableOpacity
            ...
            onPress={() => { setModalType("baru"); toggleFab(); }}
            disabled={isAnyCheckedIn}
          >
            <MaterialIcons name="person-add" size={24} color="#fff" />
          </TouchableOpacity>
        </Animated.View>
        <TouchableOpacity style={styles.fabMain} onPress={toggleFab} disabled={isAnyCheckedIn}>
          <MaterialIcons name={fabOpen ? "close" : "add"} size={28} color="#fff" />
        </TouchableOpacity>
      </Animated.View>

      {/* Modal Unscheduled */}
      <Modal visible={modalType === "unscheduled"} animationType="slide" transparent={false}>
        <SafeAreaView style={[styles.container, { paddingBottom: 24 }]} edges={["bottom", "left", "right"]}>
          {unscheduledLoading && (
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <TouchableOpacity
                style={[styles.customerItem, selectedCustomer?.id === item.id && styles.selectedCustomer]}
                onPress={() => setSelectedCustomer(item)}
              >
                <Text style={{ fontWeight: "600" }}>No Customer : {item.id}</Text>
          ...
          <View style={{ flexDirection: "row", gap: 8, marginTop: 12 }}>
            <TouchableOpacity style={styles.btnPrimary} onPress={handleUnscheduledVisit}>
              <Text style={styles.btnPrimaryText}>Mulai Kunjungan</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={[styles.btnSecondary, { backgroundColor: "#eee" }]}
              onPress={() => { setModalType(null); setSelectedCustomer(null); }}
            >
              <Text>Batal</Text>
            </TouchableOpacity>
          </View>
      ...
      {/* Modal New Customer */}
      <Modal visible={modalType === "baru"} animationType="slide" transparent={false}>
        <SafeAreaView style={[styles.container, { paddingBottom: 24 }]} edges={["bottom", "left", "right"]}>
          <Text style={styles.modalTitle}>Tambah Customer Baru</Text>
          <TextInput placeholder="Nama" style={styles.input} value={newCustomer.name} onChangeText={(t) => setNewCustomer({ ...newCustomer, name: t })} />
          <TextInput placeholder="Alamat" style={styles.input} value={newCustomer.address} onChangeText={(t) => setNewCustomer({ ...newCustomer, address: t })} />
          <TextInput placeholder="Nomor HP" style={styles.input} value={newCustomer.phone} onChangeText={(t) => setNewCustomer({ ...newCustomer, phone: t })} />
          <View style={{ flexDirection: "row", gap: 8, marginTop: 12 }}>
            <TouchableOpacity style={styles.btnPrimary} onPress={handleNewCustomerVisit}>
              <Text style={styles.btnPrimaryText}>Simpan</Text>
            </TouchableOpacity>
            <TouchableOpacity style={[styles.btnSecondary, { backgroundColor: "#eee" }]} onPress={() => setModalType(null)}>
              <Text>Batal</Text>
            </TouchableOpacity>
          </View>
        </SafeAreaView>
      ...
      {/* Modal Memo */}
      <Modal visible={memoModalVisible} animationType="slide" transparent={false}>
        <View style={styles.modalContainer}>
          ...
          />
          <View style={{ flexDirection: "row", gap: 8, marginTop: 12 }}>
            <TouchableOpacity style={styles.btnPrimary} onPress={() => saveMemo(memoText)}>
              <Text style={styles.btnPrimaryText}>Simpan</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={[styles.btnSecondary, { backgroundColor: "#eee" }]}
