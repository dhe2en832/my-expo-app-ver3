...
export default function RKSPage() {
  ...
  const [unscheduledModalVisible, setUnscheduledModalVisible] = useState(false);
  const [customerList, setCustomerList] = useState<Customer[]>([]);
  const [loadingCustomers, setLoadingCustomers] = useState(false);
  const [selectedCustomerForUnscheduled, setSelectedCustomerForUnscheduled] =
  ...
  // --- FAB State ---
  // ✅ FAB State dengan improved features
  ...
  const startPulseAnimation = () => {
    ...
    ]).start(() => startPulseAnimation()); // Loop
  };
  ...
  // ✅ FUNCTION BARU: Generate kode_rks untuk RKS tidak terjadwal
  // ✅ SOLUTION: 10 karakter total (U + 9 digit timestamp)
  const generateUnscheduledRKSCode = () => {
    const timestamp = Date.now().toString().slice(-9);
    return `U${timestamp}`; // Contoh: "U176045976"
  };

  // Untuk konsistensi, buat juga function untuk NOO:
  ...
  const handleScrollToIndexFailed = (info: {
    ...
    if (flatListRef.current) {
      ...
      console.log("🔄 Fallback scrolling to offset:", offset);

      flatListRef.current.scrollToOffset({
        offset: Math.max(0, offset - 100), // Offset ke atas sedikit
        animated: true,
      });
    }
  };
  ...
  const highlightRKSItem = (itemId: string) => {
    ...
    setTimeout(() => {
      ...
      if (flatListRef.current && itemIndex !== -1) {
        console.log("🔄 Scroll to index:", itemIndex);
        try {
          flatListRef.current.scrollToIndex({
            index: itemIndex,
            animated: true,
            viewPosition: 0.1,
            viewOffset: 70, // jika ada header
          });
        } catch (err) {
          console.warn("❌ scrollToIndex gagal:", err);
        }
      }
    }, 400);

    setTimeout(() => setHighlightedItemId(null), 8000);
  };

  // rks.tsx - Update generateNextRowId:

  const generateNextRowId = async (): Promise<number> => {
    if (!user?.kodeSales) {
      // Fallback: generate number dalam range SMALLINT
    ...
    try {
      ...
      // console.log("latest Header ", latestHeader);
      // 3. Ambil max rowid dari details header terbaru
      const maxRowIdRes = await rksAPI.getMaxRowId(latestHeader.kode_rks);

      if (maxRowIdRes.success && maxRowIdRes.maxRowId !== undefined) {
        // 4. Return rowid berikutnya (max + 1), pastikan dalam range SMALLINT
        const nextRowId = maxRowIdRes.maxRowId + 1;

        // ✅ VALIDASI: Pastikan tidak exceed SMALLINT max (32,767)
        if (nextRowId > 32767) {
          console.warn("⚠️ RowID exceed SMALLINT max, resetting to 1");
          return 1;
        }

        return nextRowId;
      } else {
        // Fallback: random number dalam range aman
        return Math.floor(Math.random() * 1000) + 1;
      }
    } catch (error) {
      console.error("Error generating next rowid:", error);
      // Fallback: random number dalam range aman
      return Math.floor(Math.random() * 1000) + 1;
    }
  };
  ...
  const handleMainFABPress = () => {
    ...
    if (isAnyCheckedIn) {
      Alert.alert(
        ...
        "Selesaikan kunjungan yang sedang berlangsung terlebih dahulu sebelum membuat yang baru.",
        [{ text: "Mengerti" }]
      );
      return;
    }
  ...
  // ✅ Handler untuk Open Order dari FAB
  const handleFABOpenOrder = () => {
    if (isAnyCheckedIn) {
      Alert.alert(
        "Kunjungan Sedang Berlangsung",
        "Selesaikan kunjungan yang sedang berlangsung terlebih dahulu.",
        [{ text: "Mengerti" }]
      );
      return;
    }
  ...
  // ✅ Update FAB handlers dengan disabled state
  const handleFABUnscheduledVisit = () => {
    if (isAnyCheckedIn) {
      Alert.alert(
        "Kunjungan Sedang Berlangsung",
        "Selesaikan kunjungan yang sedang berlangsung terlebih dahulu.",
    ...
    setFabOpen(false);
    // console.log("harusnya ngeclose");
    loadCustomerList();
    setUnscheduledModalVisible(true);
    toggleFab();
  };

  // ✅ Handler untuk Sync Data dari FAB
  const handleFABSyncData = () => {
    console.log("🔄 FAB: Sync Data");
    setFabOpen(false);
    // Panggil fungsi sync data
    handleSyncData();
  };

  // Di dalam RKSPage - tambahkan alert helper
  const showDisabledAlert = () => {
    Alert.alert(
      "Kunjungan Sedang Berlangsung",
      "Selesaikan kunjungan yang sedang berlangsung terlebih dahulu sebelum melakukan aksi ini.",
      [{ text: "Mengerti" }]
    );
  };

  // Update handlers
  const handleFABQuickCheckIn = () => {
    if (isAnyCheckedIn) {
      showDisabledAlert();
      return;
    }
    setFabOpen(false);
    Alert.alert("Quick Check-in", "Fitur quick check-in akan datang", [
      { text: "OK" },
    ]);
  };
  ...
  const handleFABNewCustomer = () => {
    ...
    setFabOpen(false);
    // ✅ NAVIGATE KE CREATE CUSTOMER DENGAN PARAMS RKS NOO
    router.push({
  ...
  const handleSyncData = async () => {
    try {
      ...
      const syncRes = await rksAPI.syncRKS(pendingRKS);
      ...
      if (syncRes.success) {
        ...
        throw new Error(syncRes.error || "Gagal sync data");
      }
    } catch (error: any) {
  ...
  // ✅ Improved toggleFab dengan stagger animation dan tooltip
  const toggleFab = () => {
    ...
    if (!fabOpen) {
      ...
      Animated.parallel([
        ...
        // Tooltip animasi (fade-in lalu hilang)
        Animated.sequence([Animated.delay(150), Animated.delay(1200)]),
      ]).start();
    } else {
      // 💠 Tutup FAB
      Animated.parallel([
  ...
  const fabActions: FABAction[] = [
    {
      ...
      icon: "add-location",
      label: "Kunjungan Tambahan",
      color: "#2196F3",
      onPress: handleFABUnscheduledVisit,
    },
    {
      id: "new-customer",
      icon: "person-add",
  ...
  const loadCustomerList = async () => {
    ...
    try {
      const customersRes = await rksAPI.getCustomerListRksUnschedule(
        user.kodeSales
      );
  ...
  const handleCustomerSelect = async (customer: Customer) => {
    setSelectedCustomerForUnscheduled(customer);

    try {
      // ✅ VALIDASI: Cek apakah customer sudah ada di RKS terjadwal hari ini
      const today = new Date().toISOString().split("T")[0];
      console.log("🔍 Validasi customer:", customer.nama_cust);
      console.log("📅 Hari ini:", today);
      console.log("📋 RKS List length:", rksList.length);

      const existingRKS = rksList.find(
        (item) =>
          item.kode_cust === customer.kode_cust &&
          item.scheduledDate === today &&
          item.isUnscheduled !== "Y" // Hanya cek yang terjadwal, bukan unscheduled
      );
      console.log("✅ Existing RKS found:", existingRKS);
      if (existingRKS) {
        // ✅ JIKA SUDAH ADA: Tampilkan alert dan highlight
        setUnscheduledModalVisible(false);

        Alert.alert(
          "Kunjungan Sudah Terjadwal",
          `Customer ${customer.nama_cust} sudah memiliki jadwal kunjungan hari ini.\n\nAplikasi akan otomatis scroll ke item tersebut dan memberikan highlight.`,
          [
            {
              text: isScrolling ? "Scrolling..." : "Tunjukkan Jadwal",
              onPress: () => {
                highlightRKSItem(existingRKS.id);
              },
            },
            {
              text: "Tetap Lanjutkan",
              style: "default",
              onPress: () => proceedWithUnscheduledVisit(customer),
            },
      ...
      await proceedWithUnscheduledVisit(customer);
    } catch (error) {
      console.error("Error in customer selection:", error);
      Alert.alert("Error", "Gagal memproses pilihan customer");
    }
  };
  ...
  const proceedWithUnscheduledVisit = async (customer: Customer) => {
    ...
    try {
      const salesCode = user?.kodeSales;
      if (!salesCode) {
        Alert.alert("Error", "Kode sales tidak ditemukan");
        return;
      }
      let selectedKodeRKS = "";
      let nextRowId = 1;

      // 1️⃣ Cek apakah hari ini sudah ada RKS master
      const masterRes = await rksAPI.getTodayRKSHeader(salesCode);
      const masterKodeRKS = masterRes?.data?.kode_rks;

      if (masterKodeRKS) {
        // Ada RKS master hari ini
        selectedKodeRKS = masterKodeRKS;

        // Ambil MAX rowid dari detail +1
        const maxRowRes = await rksAPI.getMaxRowId2(selectedKodeRKS);
        nextRowId = (maxRowRes?.data || 0) + 1;

        console.log(
          "🟢 Menggunakan RKS master:",
          selectedKodeRKS,
          "RowID:",
          nextRowId
        );
        ...
        if (unschedKodeRKS) {
          selectedKodeRKS = unschedKodeRKS;
      ...
      const uniqueId = `${Date.now()}_${Math.random()
        .toString(36)
        .substring(2, 9)}`;
      const id = `unscheduled_${uniqueId}`;
      ...
      // Langsung buka check-in setelah 500ms
      setTimeout(() => {
        handleCheckIn(unscheduledItem, "unscheduled");
      }, 500);
    } catch (error) {
      console.error("❌ Error creating unscheduled RKS:", error);
      Alert.alert("Error", "Gagal membuat kunjungan tidak terjadwal");
  ...
  const handleNewCustomerCreated = async (customerData: {
    ...
    try {
      ...
      // Tambahkan ke daftar RKS aktif
      setRksList((prev) => [nooItem, ...prev]);

      // Langsung arahkan ke check-in setelah delay 0.5s
      setTimeout(() => {
        handleCheckIn(nooItem, "noo");
      }, 500);
    } catch (error) {
      console.error("❌ Error creating NOO RKS:", error);
      Alert.alert("Error", "Gagal membuat kunjungan customer baru (NOO)");
    }
  };

  // ✅ NEW: Handle actions
  const handleUnscheduledVisit = () => {
    loadCustomerList();
    setUnscheduledModalVisible(true);
  };

  const handleOpenOrder = () => {
    console.log("📦 Open Order untuk:", recentCheckedInCustomer);
    // Navigate ke Open Order screen
    // router.push('/open-order');
    Alert.alert("Open Order", `Membuka order untuk ${recentCheckedInCustomer}`);
  };

  const handleCustomerVisit = () => {
  ...
  useEffect(() => {
    if (params.newNooCustomer) {
      try {
        const customerData = JSON.parse(params.newNooCustomer as string);
        console.log("🎯 Received new NOO customer:", customerData);
        handleNewCustomerCreated(customerData);

        // Clear params setelah diproses
        router.setParams({ newNooCustomer: undefined });
      } catch (error) {
        console.error("Error parsing NOO customer data:", error);
      }
    }
  }, [params.newNooCustomer]);
  ...
  const loadRKS = async () => {
    ...
    try {
      ...
      if (Array.isArray(rawData)) {
        ...
        // Penanganan jika API mengembalikan satu objek, diubah menjadi array
        rksArray = [rawData];
      }
      ...
      // --- START REVISI UNTUK MENGATASI DUPLIKASI KEYS ---

      // 1. Tambahkan key komposit ke setiap item RKS Master
      const mappedRKSArray = rksArray.map((item) => ({
        ...item,
        // Buat key yang sama dengan logika yang akan digunakan untuk id RKSItem
        temp_key: `master_${item.detail_rowid}_${item.rks_kode_rks}_${item.detail_kode_cust}`,
      }));

      // 2. Filter duplikat berdasarkan temp_key
      const uniqueRKSArray = removeDuplicates(mappedRKSArray, "temp_key");

      console.log(
        `🧹 Data API: ${rksArray.length} items. Setelah filter duplikat: ${uniqueRKSArray.length} items.`
      );
      ...
      // Gunakan uniqueRKSArray untuk mapping ke RKSItem
      const items: RKSItem[] = uniqueRKSArray.map((item) => {
        const scheduledDate = item.rks_tanggal.split(" ")[0];
        ...
        if (localMatch) {
          return {
            // Jika ada localMatch, gunakan ID dari data lokal (diasumsikan unik)
            id: localMatch.id,
            kode_rks: item.rks_kode_rks,
            kode_cust: item.detail_kode_cust,
            no_cust: item.no_cust,
        ...
        return {
          // Jika tidak ada localMatch, gunakan temp_key yang sudah dijamin unik
          id: item.temp_key,
          kode_rks: item.rks_kode_rks,
          kode_cust: item.detail_kode_cust,
          no_cust: item.no_cust,
  ...
  const checkFasMapAndProceed = async (
    ...
    try {
      ...
      if (
        type === "checkin" &&
        (!fasmapRes.success || !customerLocData) &&
        sumber != "noo"
      ) {
  ...
  // ✅ UPDATE: Process check-in dengan support RKS tidak terjadwal
  const processCheckIn = async (
    ...
    try {
      ...
      // ✅ PAYLOAD KHUSUS untuk RKS tidak terjadwal
      const payload = {
      ...
      // ✅ SIMPAN KE LOCAL DENGAN FLAG is_unscheduled
      await insertRKSLocal({
        ...newCheckIn,
        is_unscheduled: currentCustomer.isUnscheduled || "N", // ✅ PENTING!
        baru: currentCustomer.baru || "N",
      });
      ...
      const updatedItem: RKSItem = {
        ...
        isUnscheduled: currentCustomer.isUnscheduled, // ✅ PERTAHANKAN FLAG
      };
      ...
      await Notifications.scheduleNotificationAsync({
        content: {
          ...
          body: `Kunjungan ${
            currentCustomer.isUnscheduled === "Y"
              ? "tidak terjadwal"
              : "terjadwal"
  ...
  const handleCheckIn = async (item: RKSItem, sumber: any) => {
    await checkFasMapAndProceed(item, "checkin", sumber);
  };
  ...
  const renderItem = ({ item }: { item: RKSItem }) => {
    ...
    return (
      <Animated.View
        // key={`${item.id}_${highlightVersion}`}
        style={[
          styles.card,
          isHighlighted && styles.highlightedCard, // ✅ STYLE HIGHLIGHT
        ]}
      >
        <View
          style={{
        ...
        >
          {canCheckIn && (
            <TouchableOpacity
              ...
              onPress={() => handleCheckIn(item, "rks")}
  ...
  const SceneList = () => {
    ...
    return (
      <FlatList
        ...
        //   length: 188, // ESTIMATED_CARD_HEIGHT + CARD_MARGIN + padding
        //   offset: 188 * index,
        //   index,
        // })}
      />
  ...
  return (
    <View style={styles.container}>
      ...
      {/* ✅ POST CHECK-IN ACTIONS MODAL - Clean dan Terpisah */}
      <PostCheckInActionsModal
        visible={postCheckInModalVisible}
        onClose={() => setPostCheckInModalVisible(false)}
        customerName={recentCheckedInCustomer}
        onPayment={handlePayment}
        onStockCheck={handleStockCheck}
        onNotes={handleNotes}
        onSO={handleSO}
        onCompetitorData={handleCompetitorData}
      />

      {/* ✅ ENHANCED FAB - Menggunakan interface yang sudah ada */}

      <EnhancedFAB
        fabOpen={fabOpen}
        fabAnim={fabAnim}
        actions={fabActions}
        onMainPress={handleMainFABPress}
        // tooltipAnim={tooltipAnim}
        pan={pan}
        isAnyCheckedIn={isAnyCheckedIn}
      />

      {/* ✅ UNSCHEDULED VISIT MODAL - Menggunakan interface yang sudah ada */}
      <UnscheduledVisitModal
        visible={unscheduledModalVisible}
        onClose={() => setUnscheduledModalVisible(false)}
...
const styles = StyleSheet.create({
  ...
  // styles - Tambahkan:
  // styles - Improve highlightedCard:
  highlightedCard: {
    borderWidth: 6, // ✅ LEBIH TEBAL
    borderColor: "#FF4081", // ✅ WARNA PINK/ORANGE YANG SANGAT TERANG
    backgroundColor: "#FFF8E1", // ✅ BACKGROUND KUNING TERANG
    elevation: 20, // ✅ SHADOW LEBIH BESAR
    transform: [{ scale: 1.05 }], // ✅ SCALE EFFECT
    shadowColor: "#FF4081",
    shadowOffset: { width: 0, height: 4 },
    shadowOpacity: 0.9, // ✅ SANGAT TERANG
    shadowRadius: 20, // ✅ BESAR
    zIndex: 999,
    // ✅ TAMBAH PULSE EFFECT DENGAN ANIMATION
  },
});
