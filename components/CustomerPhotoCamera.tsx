...
const CustomerPhotoCamera: React.FC<CustomerPhotoCameraProps> = ({
  ...
  const [currentPhotoType, setCurrentPhotoType] = useState<CustomerPhoto['type']>(photoTypes[0]);
  const [capturedPhotos, setCapturedPhotos] = useState<CustomerPhoto[]>(initialPhotos);
    ...
    try {
      ...
      if (photo.base64) {
        ...
        Alert.alert(
          ...
          `Foto ${getPhotoTypeLabel(currentPhotoType)} siap untuk diupload sebagai ZIP\n\n` +
...
const handleSave = () => {
    ...
    Alert.alert(
      ...
      [
        { text: "Edit Lagi", style: "cancel" },
        {
          ...
          onPress: () => {
            console.log(`📦 Preparing ${capturedPhotos.length} photos for ZIP upload`);
  ...
  const remainingTypes = photoTypes.filter(type => !completedTypes.includes(type));
        ...
        >
          ...
          <View style={styles.watermarkContainer}>
            ...
            <Text style={styles.watermarkText}>
              {location ? `📍 ${location.coords.latitude.toFixed(6)}, ${location.coords.longitude.toFixed(6)}` : "Mendapatkan lokasi..."}
            </Text>
            {location && (
              <View style={[styles.accuracyContainer, { backgroundColor: getAccuracyColor(location.coords.accuracy || 999) }]}>
            ...
            <Text style={styles.watermarkType}>
              {getPhotoTypeLabel(currentPhotoType)} {getPhotoTypeIcon(currentPhotoType)}
            </Text>
          </View>
              ...
              {photoTypes.map((type) => (
                <TouchableOpacity
                  ...
                  style={[
                    ...
                    completedTypes.includes(type) && styles.photoTypeButtonCompleted,
                  ]}
                  onPress={() => setCurrentPhotoType(type)}
                >
                  <Text style={styles.photoTypeIcon}>{getPhotoTypeIcon(type)}</Text>
                  <Text style={[
                    styles.photoTypeText,
                    currentPhotoType === type && styles.photoTypeTextActive,
                    completedTypes.includes(type) && styles.photoTypeTextCompleted,
                  ]}>
...
const styles = StyleSheet.create({
  ...
  permissionButton: { backgroundColor: "#667eea", padding: 15, borderRadius: 8 },
  ...
  watermarkTitle: {
    color: "#fff", fontSize: 16, fontWeight: "bold", marginBottom: 5, textAlign: "center",
  },
  watermarkText: { color: "#fff", fontSize: 14, marginBottom: 3 },
  watermarkType: {
    color: "#ffeb3b", fontSize: 16, fontWeight: "bold", marginTop: 5, textAlign: "center",
  },
  accuracyContainer: {
    flexDirection: "row", alignItems: "center",
    paddingHorizontal: 10, paddingVertical: 6, borderRadius: 6, marginVertical: 5,
  ...
  photosPreviewTitle: {
    color: "#fff", fontSize: 14, fontWeight: "bold", marginBottom: 8,
  },
  photoPreviewItem: { alignItems: "center", marginHorizontal: 5 },
  photoPreviewImage: { width: 60, height: 60, borderRadius: 8 },
  removePhotoButton: {
    position: "absolute", top: -5, right: -5,
    backgroundColor: "#f44336", borderRadius: 10, padding: 2,
  },
  photoPreviewLabel: {
    color: "#fff", fontSize: 10, marginTop: 4, textAlign: "center",
  },
  saveButton: {
    position: "absolute", bottom: 50, left: 20, right: 20,
    backgroundColor: "#4CAF50", padding: 15, borderRadius: 8, alignItems: "center",
  },
