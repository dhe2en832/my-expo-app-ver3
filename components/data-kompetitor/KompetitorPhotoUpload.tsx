// components/kompetitor/KompetitorPhotoUpload.tsx
...
import { KompetitorPhoto } from "@/api/interface";

interface KompetitorPhotoUploadProps {
        ...
        {permission?.granted ? (
          <CameraView style={styles.camera} facing="back" ref={onCameraRef}>
            <View style={styles.cameraOverlay}>
              <Text style={styles.cameraTitle}>Ambil Foto Data Kompetitor</Text>
              <Text style={styles.cameraSubtitle}>
                Pastikan product terlihat jelas
              </Text>
            </View>
...
const KompetitorPhotoUpload: React.FC<KompetitorPhotoUploadProps> = ({
  photos,
  onPhotosChange,
  maxPhotos = 5,
  disabled = false,
  ...
  const launchImageLibrary = useCallback(async () => {
    try {
      ...
      if (!result.canceled && result.assets) {
        const newPhotos: KompetitorPhoto[] = result.assets.map(
          (asset, index) => ({
            id: `kompetitor-photo-${Date.now()}-${index}`,
            uri: asset.uri,
            name: `bukti_harga_kompetitor_${Date.now()}_${index}.jpg`,
            type: "image/jpeg",
            base64: asset.base64 || undefined,
            filename: `bukti_harga_kompetitor_${Date.now()}_${index}.jpg`,
  ...
  }, [launchCamera, launchImageLibrary, disabled]);
      ...
      try {
        ...
        if (photo.base64) {
          const newPhoto: KompetitorPhoto = {
            id: `photo_${Date.now()}`,
            uri: photo.uri,
            name: `bukti_harga_kompetitor_${Date.now()}.jpg`,
            type: "image/jpeg",
            base64: photo.base64,
            filename: `bukti_harga_kompetitor_${Date.now()}.jpg`,
            timestamp: new Date().toISOString(),
          };
          ...
          Alert.alert(
            ...
            `Foto data harga kompetitor berhasil diambil\nTotal foto: ${
              photos.length + 1
            }/${maxPhotos}`,
            [{ text: "Lanjutkan" }]
          );
      ...
      <Card style={styles.photoCard}>
        <Card.Content>
          <View style={styles.photoHeader}>
            <Text style={styles.photoTitle}>Foto Data Kompetitor</Text>
...
export default KompetitorPhotoUpload;