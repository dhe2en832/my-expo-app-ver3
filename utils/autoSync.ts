...
class SyncProgressTracker {
    ...
    setWeights(weights: number[]) {
        ...
        if (weights.length === this.stepNames.length && weights.reduce((a, b) => a + b, 0) === 100) {
...
/**
 * Fungsi inti untuk menyinkronkan data yang tertunda (RKS & Customer)
...
const syncPendingRecords = async (tracker: SyncProgressTracker | null) => {
    try {
        ...
        const totalRecords = pendingRKS.length + pendingCustomers.length;
        let syncedCount = 0;

        // Jika ada tracker, laporkan langkah sync data
        if (tracker) {
            tracker.startStep("Sinkronisasi Transaksi (RKS & Customer)"); // Resolves Error 1
        }
        ...
        const updateProgress = (tableName: string) => {
            ...
            if (tracker) {
                ...
                tracker.updateMessage(`Sync ${tableName}: ${syncedCount}/${totalRecords} records`);
            }
        };
...
export const performInitialFullSync = async () => {
    ...
    const tracker = new SyncProgressTracker(["Sinkronisasi Transaksi"]);
